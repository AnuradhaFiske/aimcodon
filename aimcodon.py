...
if not st.session_state.logged_in:
    ...
    if login_button:
        if mobile_number and gmail_account and x_account:
...
if st.session_state.logged_in:
    ...
    if st.button("Review Order"):
        if not (name and contact_number and address):
            st.error("Please fill in all profile details before reviewing the order.")
        else:
    ...
    # Billing Calculation
    def calculate_billing(vegetable_cost):
        chef_cost = 300
        service_cost = 200
        ...
        return chef_cost, service_cost, other_ingredients_cost, delivery_cost, subtotal, sgst, cgst, total_cost
    ...
    if st.session_state.reviewed_order:
        ...
        costs = calculate_billing(vegetable_cost)
        chef_cost, service_cost, other_ingredients_cost, delivery_cost, subtotal, sgst, cgst, total_cost = costs
